...
def daily_report(df):
    ...
    variance=sum((mean-x)**2 for x in today)/len(today)
    volatility=(variance**(1/2))
    return ("Daily low : " + str(round(low,2)) + ' | ' +  "Daily high : " + str(round(high,2))  + ' | ' +  "Daily mean : " + str(round(mean,2))  + ' | ' + "Daily volatility : " + str(round(volatility,2)) + " %")
...
def create_app():
    ...
    def update_timeseries(n):
                ...
                fig = px.line(df, x='Time (UTC)', y='EUR/KRW', title='EUR/KRW rate')
                return fig
